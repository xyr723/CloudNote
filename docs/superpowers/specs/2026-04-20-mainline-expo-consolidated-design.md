...
- 不在本轮把 widget 表单编辑也完全下沉到纯 DOM 内联体验
- 不引入新的线上部署或 EAS 发布链路
...
经过 Chunk 17 收口后，上一轮剩下的两项高风险边界都已经落地：

1. **`document-first` state handler 已贯通到媒体 / 录音 / AI / 主要格式化链路**
   - controller 现在会把 formatting / media / recording 链路产生的 granular patch 在同 tick 合并成一份统一 state，再一次性同步 `content / textSegments / images / audios / fontSize / document`
   - AI 续写不再走 `onAppendText + onAppendWidgets` 两条分叉，而是改成单一 completion handler，统一生成下一份 `textSegments + document` 再提交
   - 原生输入、H5 block-aware bridge、媒体 marker 更新、录音插入、AI 续写和主要格式化操作，当前都已经在同一个 `document-first` 入口上收口
   - 这样 `native + content + textSegments` 与 `H5 + document` 的双轨漂移风险已经进一步压缩到单一 handler 内部，而不是散落在多个 feature hook 里

2. **Web 端已切到统一 H5 编辑宿主**
   - Web H5 editor 现在不再使用 RN `TextInput` shell，而是改成 `iframe + srcDoc`
   - 它复用了与移动端相同的 `createH5TextEditorHtml / createH5TextEditorSyncScript / createH5TextEditorFormatScript / parseH5TextEditorMessage`
   - bridge 现在同时支持 `ReactNativeWebView.postMessage` 和 `window.parent.postMessage`，因此移动端 `WebView` 与 Web `iframe` 可以共享同一套 HTML bridge 协议
   - Web 侧的 media insert / selection / widget action / content-change 现在都通过同一套 bridge message 回到 controller，不再保留独立的 web shell 行为

当前主链剩余的高风险边界缩减为两项：

1. **widget 表单是否继续完全下沉到纯 DOM 内联体验**
   - 当前 widget 的类型选择、编辑与删除编排已经统一接到 H5 bridge
   - 但复杂表单仍在宿主外层 sheet 中完成，尚未完全变成 iframe 内原位编辑

2. **Web / PWA 更大范围回归验证**
   - 当前已验证 Web bundle、静态导出和核心 H5 编辑链路
   - 但还没有完成更大规模的浏览器 / 真机 / 发布级验证
